...
require 'rom/auth/migrator'
require 'rom/auth/migrations/migration'
require 'rom/auth/migrations/users_migration'
require 'rom/auth/migrations/authentication_events_migration'
require 'rom/auth/migrations/authentication_credentials_migration'
  ...
  module Auth
    class << self
      ...
      def load_plugins!
        ...
        configuration.plugins.each do |name, args|
          plugin = available_plugins[name].new(self, *args)

          @loaded_plugins.merge!(name => plugin)
        end
      end

      def install_plugins!
        @loaded_plugins.each do |_name, plugin|
          plugin.install
        end
      end
      ...
      def migrate(setup)
        #Migrator.migrate!(configuration, setup)
        loaded_plugins.each do |_name, plugin|
      ...
      def authenticate(type, user, data)
        ...
        authenticated = authenticators[type].new.authenticate(user, data)
        success = false
